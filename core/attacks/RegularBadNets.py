...
class PoisonedRegularizationMnist(VisionDataset):
    """
    ...
    def __init__(self,
                 ...
                 weight,
                 poisoned_transform_index):
        super(PoisonedRegularizationMnist, self).__init__(
            benign_dataset.root,
    ...
    def __getitem__(self, index):
        ...
        img = Image.fromarray(img)
        if index in self.poison_indices or index in self.cover_indices:
            img = self.poisoned_transform(img)
    ...
    def set_poisoned_subdatasets(self, y_target = None, poisoning_rate = 0.0):
        ...
        self.cover_indices = sorted(list(tmp_list[poisoned_num:poisoned_num+int(poisoned_num*self.cover_rate)]))
...
class RegularBadNets(Base, Attack):
    """
    ...
    def create_poisoned_dataset(self, dataset, y_target=None, poisoning_rate = None):
        ...
        if dataset_type == MNIST:
            return PoisonedRegularizationMnist(benign_dataset, y_target, poisoning_rate, cover_rate, pattern, weight, poisoned_transform_index)
